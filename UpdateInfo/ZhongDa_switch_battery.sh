...
######################################直流告警#######################################
	...
	basic.exe zhongda.txt 2 100 20 $1
	recive_dc_alarm=$(cat receiveinfo)
	
	
	alarm_ascii=${recive_dc_alarm:66:8} ##电池熔丝故障：
	alarm_hex=$(echo $alarm_ascii | sed 's/\(..\)/\\\\x\1/g' | xargs echo -e)

	alarm_binary=$( hex4_binary16  "$alarm_hex") 

	echo "alarm_binary"$alarm_binary
	dc_voltage_output_lower=0    ###0表示正常直流输出电压过低告警<47
	dc_voltage_output_lower=${alarm_binary:7:1}
	
	if [ "$dc_voltage_output_lower" == "1" ];then
	...
	dc_voltage_output_higher=${alarm_binary:6:1}
	if [ "$dc_voltage_output_higher" == "1" ];then
	...
	battery_melt_alarm=${alarm_binary:10:1} 
	if [ "$battery_melt_alarm1" == "1" ];then
	...
	charge_current_over1=${alarm_binary:12:1}   ##电池1组充电过流故障
	if [ "$charge_current_over1" == "1" ];then
	...
	charge_temperature_over1=${alarm_binary:13:1} ##电池组1温度过高故障
	if [ "$charge_temperature_over1" == "1" ];then
	...
	basic.exe zhongda.txt 5 100 20  $1
	...
	ac_voltage_input_diff1=${recive_ac_alarm:38:4}
	...
	ac_voltage_input_diff2=${recive_ac_alarm:42:4}
	...
	ac_voltage_input_diff3=${recive_ac_alarm:46:4}
	...
	basic.exe zhongda.txt 8 100 18 $1
	...
	configure_module_communication_status1=${recive_rectifier_alarm:46:4}
...
######################################交流测量##################################################
	...
	basic.exe zhongda.txt 11 100 20 $1
	...
	basic.exe zhongda.txt 14 100 20 $1
	...
	spower_group1_current_ascii=${recive_dc_analog_output:66:16}
	...
	#################################整流测量##########################
	##############################################################################
	basic.exe zhongda.txt 17 100 18 $1
	...
	rectifier_module_temperature_ascii=${recive_rectifier_analog_output:50:16}
