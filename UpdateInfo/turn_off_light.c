...
{
	//char *dev  = "/dev/ttyS15"; //串口

	int fd = OpenDev(argv[1]);           //打开串口函数
	if( fd < 0 )
        {
            return FALSE;
        }
	...
	char order[21]={0xfe,0xfe,0x68,0xa0,0xa0,0xa0,0xa0,0xa0,0xa0,
             0x68,0x04,0x07,0x0a,0x0a,0xaa,0xaa,0xaa,0xaa,0x10,0x67,0x16};


	int len;
        tcflush(fd,TCIOFLUSH);

	len=write(fd,order,21);
	if(len==21)
	{
	    puts("Close the light successful!");
	}             
