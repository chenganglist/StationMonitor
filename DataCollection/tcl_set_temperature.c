...
{
	int realtemperature =  temperature - 18;
	
	unsigned char temperature_byte = (unsigned char)realtemperature;
	
	printf("16进制温度控制码为：%02x,其10进制为%d\n",
	temperature_byte,temperature_byte);
	
	unsigned char str[10] = {0x09, 0xFE ,0x01, 0x1F,
	   0x00, 0x00, 0x21 , temperature_byte, 0x00, 0x00};
	...
	{
		sum = sum + str[i];
		printf("第%d个字符,16进制为%02X\n",i,str[i]);
	}  
	printf("校验和为%02X\n",sum);
	
	unsigned char order[15] = {0xF4, 0xF5, 0x09, 0xFE, 0x01, 0x1F,
	 0x00, 0x00 ,0x21, temperature_byte, 0x00, 0x00, sum, 0xF4, 0xFB};
 
	for(i=0;i<15;i++)
	{
		outcommand[i] = order[i];
		printf("第%d个命令字符,16进制为%02X\n",i,str[i]);
	}  
...
int main(int argc,char* argv[])
{
    ...
    //*********发送命令************** 
	int send_res = Send_order(fd,outcommand,15);
