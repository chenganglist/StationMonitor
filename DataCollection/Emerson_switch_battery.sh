...
######################################直流告警#######################################
	...
	basic.exe emerson.txt 2 100 18 $1
	recive_dc_alarm=$(cat receiveinfo)
	##电池1熔丝故障
	battery_melt_alarm1=${recive_dc_alarm:82:4}
	
	
	if [ "$battery_melt_alarm1" == "3033" ];then
		battery_melt_alarm=1
	fi
#	if [ "$battery_melt_alarm" -ne 1 ];then
	##电池2熔丝故障
		battery_melt_alarm2=${recive_dc_alarm:86:4}
		
		if [ "$battery_melt_alarm2" == "3033" ];then
		battery_melt_alarm=1
		fi
#	fi
#	if [ "$battery_melt_alarm" -ne 1 ];then
	##电池3熔丝故障
		battery_melt_alarm3=${recive_dc_alarm:90:4}
		
		if [ "$battery_melt_alarm3" == "3033" ];then
			battery_melt_alarm=1
		fi
#	fi	
#	if [ "$battery_melt_alarm" -ne 1 ];then
	##电池4熔丝故障
		battery_melt_alarm4=${recive_dc_alarm:94:4}
		echo "test："$battery_melt_alarm4
		if [ "$battery_melt_alarm4" == "3033" ];then
			battery_melt_alarm=1
		fi
#	fi	
#	if [ "$battery_melt_alarm" -ne 1 ];then

		battery_melt_alarm5=${recive_dc_alarm:184:4}  ##电池5熔丝故障
	
		if [ "$battery_melt_alarm5" == "3033" ];then
			battery_melt_alarm=1
		fi
#	fi	
#	if [ "$battery_melt_alarm" -ne 1 ];then
		battery_melt_alarm6=${recive_dc_alarm:188:4}
		##电池6熔丝故障
		
		if [ "$battery_melt_alarm6" == "3033" ];then
			battery_melt_alarm=1
		fi
#	fi	
	...
	charge_current_over=0    
	###0表示正常电池充电过流告警
	charge_current_over1=${recive_dc_alarm:98:4}
	##电池1组充电过流故障
	if [ "$charge_current_over1" == "3032" ];then
		charge_current_over=1;
	fi
	if [ "$charge_current_over" -ne 1 ];then
		charge_current_over2=${alarm_recive:102:4}
		##电池2组充电过流故障
		
		if [ "$charge_current_over2" == "3033" ];then
			battery_melt_alarm=1
		fi
	fi	
	...
	charge_temperature_over1=${recive_dc_alarm:122:4} ##电池组1温度过高故障
	if [ "$charge_temperature_over1" == "4532" ];then
		charge_temperature_over=1;
	fi
	if [ "$charge_temperature_over" -ne 1 ];then
		charge_temperature_over2=${recive_dc_alarm:126:4}  ##电池2组温度过高故障
		if [ "$charge_temperature_over2" == "4532" ];then
			charge_temperature_over=1
		fi
	fi
	...
	supply_alarm1=${recive_dc_alarm:154:4} ##蓄电池供电状态故障
	if [ "$supply_alarm1" == "4535" ];then
		supply_alarm=1;
	fi
	...
	dc_voltage_output_diff=${recive_dc_alarm:170:4}
	if [ "$dc_voltage_output_diff" == "3031" ];then
	...
	if [ "$dc_voltage_output_diff" == "3032" ];then
	...
	basic.exe emerson.txt 5 100 20 $1
	...
	if [ "$ac_input_stopped1" == "4531" ];then
		ac_input_stopped=1;
	fi
	...
	lightning_arrester_alarm1=${recive_ac_alarm:66:4}
	if [ "$lightning_arrester_alarm1" == "3035" ];then
	...
	basic.exe emerson.txt 8 100 18 $1
	...
	configure_module_communication_status1=${recive_rectifier_alarm:50:4} ##蓄电池交流输入电压过高故障
	if [ "$configure_module_communication_status1" == "4532" ];then
...
######################################交流测量##################################################
	...
	basic.exe emerson.txt 14 100 20 $1
...
#	if [ "$switch_power_voltage_A" -eq 0 ];then
#		switch_power_voltage_ascii_a1=${recive_ac_analog_output:86:16}
#		switch_power_voltage_A=$(DataF_convert_Uabc  "$switch_power_voltage_ascii_a1")
...
#	if [ "$switch_power_voltage_B" -eq 0 ];then
#		switch_power_voltage_ascii_b1=${recive_ac_analog_output:102:16}
#		switch_power_voltage_B=$(DataF_convert_Uabc  "$switch_power_voltage_ascii_b1")	
#	fi
...
#	if [ "$switch_power_voltage_C" -eq 0 ];then
#		switch_power_voltage_ascii_c1=${recive_ac_analog_output:118:16}
#		switch_power_voltage_C=$(DataF_convert_Uabc  "$switch_power_voltage_ascii_c1")	
#	fi
	...
	basic.exe emerson.txt 17 100 18 $1
	...
	spower_group1_current_ascii=${recive_dc_analog_output:394:16}
	echo " test: "$spower_group1_current_ascii
	...
	spower_group2_current_ascii=${recive_dc_analog_output:410:16}
	echo " test: "$spower_group2_current_ascii
	spower_group2_current=$(DataF_convert_Uabc  "$spower_group2_current_ascii")
	
	echo "正常电池组2电流："$spower_group2_current
	...
	#################################整流测量##########################
	##############################################################################
	basic.exe emerson.txt 20 100 18 $1
	...
	echo " test: "$rectifier_module_current_ascii
	...
	rectifier_module_temperature_ascii=${recive_rectifier_analog_output:98:16}
	echo " test: "$rectifier_module_temperature_ascii
