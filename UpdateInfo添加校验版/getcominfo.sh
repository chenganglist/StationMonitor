...
####         根据最小二项式进行拟合得出最佳修正表达式，即填上offset=0.3222###
####第一次修改时间：2015年10月26日###修改人：肖良平#################################
####第一次修正：(120.0/1024.0)*1.01*$1-40.0+0.3222
####第二次修正：修正时间2015年12月9日 
####内容表达式换：(118.950/1024.0)*1.01*$1-40.0 +0.23#########################
function calculate_temperature_two()
{
	echo "scale=8; (118.950/1024.0)*1.01*$1-40.0+0.23 " | bc -l  | awk '{printf ("%8.4f",$0)}'
}
...
##############计算湿度二######################################################
####修改部分：由于AD参考电压为5.05V所以转换标准值5V之后，需要乘以1.01#########
####原本表达式：(100/1024)*1.01*$1 根据最小二项式进行拟合得出最佳修正表达式###
####第一次修正：(99.95/1024)*1.01*$1+0.276
####第二次修正：修正时间2015年12月9日 
####内容表达式换：(99.1/1024)*1.01*$1+0.1952#########################
####修改时间：2015年10月27日###修改人：肖良平#################################
function calculate_humidity_two()
{
	echo "scale=8; (99.1/1024)*1.01*$1+0.1952" | bc -l  | awk '{printf ("%8.4f",$0)}'
}
...
{
	echo "scale=8; (72/818.2)*($1-204.8)+$1/2200" | bc -l  | awk '{printf ("%8.4f",$0)}'
	#| awk '{printf "%.4f", $0}'
}
##############后半组
function calculate_battery_one2()
{
	echo "scale=8; (72/818.2)*($1-204.8)+$1/2600" | bc -l  | awk '{printf ("%8.4f",$0)}'
	#| awk '{printf "%.4f", $0}'
}
...
{
	echo "scale=8; (70/1023)*$1+$1/2100" | bc -l | awk '{printf ("%8.4f",$0)}'
}
##############后半组
function calculate_battery_two2()
{
	echo "scale=8; (70/1023)*$1+$1/750" | bc -l | awk '{printf ("%8.4f",$0)}'
}
...
if [ $arminfo == "Failed" ];then
	echo "Data is chaos,Try again Please!"
	echo "Wrong" >>  normalRate.log
    exit 1;
else
	echo "Right" >>  normalRate.log
	size=$(du -sh normalRate.log | grep M)
	if [ -n "$size" ];then
		cp normalRate.log  $(echo $(echo normalRate)$(date +%Y%m%d-%H%M%S))
		rm normalRate.log
		touch normalRate.log
	fi
fi
...
#############温度一号###################
	...
	if [  $(echo "$decimal_temperature2 < -39.67"|bc) -eq 1 ];then
		echo  "-40"  >  decimal_temperature2
	fi
	if [  $(echo "$decimal_temperature2 > 79.9"|bc) -eq 1 ];then
		echo  "80"  >  decimal_temperature2
	fi
	...
	if [  $(echo "$decimal_humidity2 < 0.278"|bc) -eq 1 ];then
		echo  "0"  >  decimal_humidity2
	fi
	if [  $(echo "$decimal_humidity2 > 99.5"|bc) -eq 1 ];then
		echo  "100"  >  decimal_humidity2
	fi
	...
	voltage=${arm:20:4}
	v=$(convert_hex_to_decimal   "$voltage")
	backward_half_battery_voltage1=$(calculate_battery_one2   "$v")
	...
	voltage=${arm:28:4}
	v=$(convert_hex_to_decimal   "$voltage")
	backward_half_battery_voltage2=$(calculate_battery_two2   "$v")
