...
######################################直流告警#######################################
	...
	basic.exe zhongxing.txt 2 100 20 $1
	...
	charge_current_over1=${recive_dc_alarm:156:4} ##电池1组充电过流故障
	if [ "$charge_current_over1" == "3830" ];then
	...
	charge_temperature_over1=${recive_dc_alarm:148:4} ##电池组1温度过高故障
	if [ "$charge_temperature_over1" == "3830" ];then
	...
	supply_alarm1=${recive_dc_alarm:180:4} ##蓄电池供电状态故障
	if [ "$supply_alarm1" == "3830" ];then
		supply_alarm=1;
	fi
	...
	basic.exe zhongxing.txt 5 100 20  $1
	...
	lightning_arrester_alarm1=${recive_ac_alarm:58:4}
	if [ "$lightning_arrester_alarm1" == "4531" ];then
	...
	basic.exe zhongxing.txt 8 100 18 $1
...
######################################交流测量##################################################
	...
	basic.exe zhongxing.txt 14 100 20 $1
	...
	switch_power_voltage_A=$(convert_hex2    "$switch_power_voltage_hex_a"  "1" )
	...
	switch_power_voltage_B=$(convert_hex2    "$switch_power_voltage_hex_b"  "1" )
	...
	switch_power_voltage_C=$(convert_hex2    "$switch_power_voltage_hex_c"  "1" )
	...
	basic.exe zhongxing.txt 17 100 20 $1
	...
	switch_power_dc_voltage=$(convert_hex2    "$switch_power_dc_voltage_hex"  "0.1" )
	...
	spower_group1_current_ascii=${recive_dc_analog_output:122:8}
	...
	# spower_group2_current_ascii=${recive_dc_analog_output:310:16}
	...
	#################################整流测量##########################
	##############################################################################
	basic.exe zhongxing.txt 20 100 18 $1
	recive_rectifier_analog_output=$(cat receiveinfo)
	
	rectifier_module_current=0    ###正常整流模块电流
	rectifier_module_current_ascii=${recive_rectifier_analog_output:30:8}
	...
	rectifier_module_temperature_ascii=${recive_rectifier_analog_output:42:8}
